...
const Avatar = ({ username, size = 32, hasStory = false }) => {
  const colors = ['#f09433','#e6683c','#dc2743','#cc2366','#bc1888','#8a3ab9','#4c68d7','#cd486b'];
...
const StoryItem = ({ username, isOwn = false }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer', minWidth: 66 }}>
    <div style={{ position: 'relative' }}>
      ...
      {isOwn && (
        <div style={{
          ...
          backgroundColor: '#0095f6', border: '2px solid #fff',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 16, color: '#fff', lineHeight: 1,
        }}>+</div>
      )}
    </div>
    <span style={{ fontSize: 12, color: '#262626', maxWidth: 66, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'center' }}>
...
const PostCard = ({ post, onLike }) => {
  ...
  return (
    <div style={{ backgroundColor: '#fff', borderBottom: '1px solid #dbdbdb', marginBottom: 0 }}>

    
      <div style={{ display: 'flex', alignItems: 'center', padding: '8px 4px 8px 12px', gap: 10 }}>
        <Avatar username={post.username || 'user'} size={32} hasStory />
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: '600', color: '#262626', cursor: 'pointer' }}>
            {post.username || 'user'}
          </div>
          {post.location && (
            <div style={{ fontSize: 12, color: '#262626' }}>{post.location}</div>
          )}
        </div>
      ...
      <div
        style={{ position: 'relative', width: '100%', paddingBottom: '100%', cursor: 'pointer', overflow: 'hidden' }}
      ...
      >
        ...
        <div style={{
          ...
          {post.media_url ? (
            ...
            <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.8)', padding: 20 }}>
      ...
      <div style={{ padding: '6px 12px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 4 }}>
          <button
            onClick={handleLike}
            style={{
              background: 'none', border: 'none', cursor: 'pointer', padding: '6px 6px 6px 0',
        ...
        {post.caption && (
          <div style={{ fontSize: 14, color: '#262626', marginBottom: 4, lineHeight: '18px' }}>
        ...
        {post.comment_count > 0 && (
          <div style={{ fontSize: 14, color: '#8e8e8e', marginBottom: 4, cursor: 'pointer' }}>
            View all {post.comment_count} comments
          </div>
        )}

        <div style={{ fontSize: 10, color: '#8e8e8e', textTransform: 'uppercase', letterSpacing: '0.2px', marginBottom: 8 }}>
          {timeAgo(post.created_at)}
        </div>
      </div>

      {showComment && (
        <div style={{ borderTop: '1px solid #dbdbdb', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <input
            ...
            style={{
              ...
              fontSize: 14, color: '#262626', backgroundColor: 'transparent',
            }}
          />
          {comment && (
            <button
              style={{ background: 'none', border: 'none', color: '#0095f6', fontWeight: '600', fontSize: 14, cursor: 'pointer' }}
              onClick={async () => {
                try {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: '600', color: '#262626', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{username}</div>
        <div style={{ fontSize: 12, color: '#8e8e8e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{subtitle}</div>
      </div>
      <button
...
export default function Feed() {
  ...
  const storyUsers = ['bob', 'charlie', 'diana', 'eve', 'frank', 'grace', 'henry'];
    ...
    <div style={{ backgroundColor: '#fafafa', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            flex: 1, maxWidth: 268, margin: '0 32px',
            ...
            <input
              ...
              style={{
                ...
                fontSize: 14, color: '#262626', width: '100%',
              }}
            />
          </div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, maxWidth: 614, minWidth: 0 }}>
          ...
          <div style={{
            ...
            borderRadius: 3, padding: '16px 0', marginBottom: 24, overflowX: 'auto',
          }}>
            <div style={{ display: 'flex', gap: 16, padding: '0 16px', width: 'max-content' }}>
              <StoryItem username={user?.username} isOwn />
              {storyUsers.map(u => <StoryItem key={u} username={u} />)}
            </div>
          </div>

          {loading && posts.length === 0 ? (
            [1, 2, 3].map(i => (
              <div key={i} style={{ backgroundColor: '#fff', border: '1px solid #dbdbdb', borderRadius: 3, marginBottom: 24 }}>
                ...
                <div className="shimmer" style={{ width: '100%', paddingBottom: '100%' }} />
            ...
            <div style={{
              backgroundColor: '#fff', border: '1px solid #dbdbdb',
              borderRadius: 3, padding: 60, textAlign: 'center',
            }}>
              <div style={{ fontSize: 48, marginBottom: 16 }}>📸</div>
              <h3 style={{ color: '#262626', marginBottom: 8 }}>No posts yet</h3>
        ...
        <div style={{ width: 293, flexShrink: 0, position: 'sticky', top: 84 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 14, color: '#8e8e8e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {user?.full_name}
              </div>
            </div>
            <button
              onClick={logout}
              style={{ background: 'none', border: 'none', color: '#0095f6', fontSize: 12, fontWeight: '600', cursor: 'pointer', flexShrink: 0 }}
            >
              Log out
            </button>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <span style={{ fontSize: 14, fontWeight: '600', color: '#8e8e8e' }}>Suggested for you</span>
            <button style={{ background: 'none', border: 'none', fontSize: 12, fontWeight: '600', color: '#262626', cursor: 'pointer' }}>
              See All
            </button>
          </div>

          {suggestedUsers.map(u => (
            <SuggestedUser key={u.username} username={u.username} subtitle={u.subtitle} />
          ))}
