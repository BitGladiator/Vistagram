...
const GridIcon = ({ active }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill={active ? '#262626' : 'none'} stroke="#262626" strokeWidth="1.5">
...
const TaggedIcon = ({ active }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={active ? '#262626' : '#8e8e8e'} strokeWidth="1.5">
...
export default function Profile() {
  ...
  const handleFollow = async () => {
    ...
    setStats(s => ({ ...s, followers: wasFollowing ? s.followers - 1 : s.followers + 1 }));
    try {
      if (wasFollowing) {
    ...
    <div style={{ backgroundColor: '#fafafa', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 935, margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{
          ...
          gap: 80, padding: '40px 0 24px',
        }}>
         
          {loading ? (
            <Skeleton width={150} height={150} borderRadius="50%" />
          ) : (
            <Avatar username={profile?.username || currentUser?.username} size={150} />
          )}
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 20, flexWrap: 'wrap' }}>
              {loading ? (
                ...
                <h2 style={{ fontSize: 28, fontWeight: '300', color: '#262626' }}>
              ...
              {isOwnProfile ? (
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    onClick={logout}
                    style={{ ...styles.outlineBtn, color: '#ed4956', borderColor: '#ed4956' }}
                  >
                    Log out
                  </button>
                </div>
            ...
            <div style={{ display: 'flex', gap: 40, marginBottom: 20 }}>
              {loading ? (
                [1,2,3].map(i => <Skeleton key={i} width={80} height={40} />)
              ) : (
                <>
                  <div>
                  ...
                  <div style={{ cursor: 'pointer' }}>
                    <span style={styles.statNum}>{stats.followers.toLocaleString()}</span>
                    <span style={styles.statLabel}> followers</span>
                  </div>
                  <div style={{ cursor: 'pointer' }}>
                    <span style={styles.statNum}>{stats.following.toLocaleString()}</span>
            ...
            {loading ? (
              ...
              <div>
                <p style={{ fontSize: 14, fontWeight: '600', color: '#262626', marginBottom: 4 }}>
                  {profile?.full_name || currentUser?.full_name}
                </p>
                {profile?.bio && (
                  <p style={{ fontSize: 14, color: '#262626', lineHeight: '18px', whiteSpace: 'pre-wrap' }}>
                    {profile.bio}
                  </p>
                )}
                {profile?.website && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    style={{ fontSize: 14, color: '#00376b', fontWeight: '600' }}
                  >
                    {profile.website}
                  </a>
                )}
        ...
        <div style={{
          display: 'flex', borderTop: '1px solid #dbdbdb',
          marginTop: 8,
        }}>
          <button
            ...
            style={{
              ...styles.tab,
              borderTop: activeTab === 'grid' ? '1px solid #262626' : '1px solid transparent',
          ...
          <button
            ...
            style={{
              ...styles.tab,
              borderTop: activeTab === 'tagged' ? '1px solid #262626' : '1px solid transparent',
              ...
              <div style={styles.emptyState}>
                ...
                {isOwnProfile ? (
                  ...
                  <>
                    ...
                    <p style={styles.emptyText}>When they share photos, you'll see them here.</p>
                  </>
                )}
              </div>
            ) : (
        ...
        {activeTab === 'tagged' && (
          <div style={styles.emptyState}>
            ...
            <p style={styles.emptyText}>When people tag you in photos, they'll appear here.</p>
          </div>
        )}
...
const styles = {
  ...
  emptyIcon: {
    fontSize: 64,
    width: 96, height: 96,
    border: '3px solid #262626', borderRadius: '50%',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    marginBottom: 16,
  },
  emptyTitle: {
  ...
  emptyText: {
    fontSize: 14, color: '#262626', maxWidth: 280,
  },
  shareFirstBtn: {
    background: 'none', border: 'none',
    color: '#0095f6', fontWeight: '600',
